...
@endif
@if(auth('admin')->user()->can('utilities-list') || auth('admin')->user()->can('utilities-create'))
    <li class="has_sub">
        <a href="javascript:void(0);" class="waves-effect"><i
                class="zmdi zmdi-collection-text"></i><span> المرافق </span> </a>
        <ul class="list-unstyled">
            @if(auth('admin')->user()->can('utilities-list'))
                <li><a href="{{route('admin.utilities.index')}}">جميع المرافق</a></li>
            @endif
            @if(auth('admin')->user()->can('utilities-create'))
